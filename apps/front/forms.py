...
class SigninFrom(BaseForm):
    telephone=StringField(validators=[Regexp('^1[35678]\d{9}$',message='请输入正确的电话号码')])
    password=StringField(validators=[InputRequired(message='必须输入密码'),Length(min=6,max=20,message='密码长度在6到20位')])
...
class AddPostForm(BaseForm):
    title = StringField(validators=[InputRequired(message="帖子标题不能为空")])
    bank_id = StringField(validators=[InputRequired(message="帖子版块不能为空")])
